...
export default async function Table3() {
  ...
  // Header data
  const tableHeaders = [
    { label: dict.bonusPolicy.other.diligence.table.headers[0], props: { rowSpan: 2, className: "border border-black text-center font-bold w-fit lg:w-[200px] h-24 text-white px-2 md:px-4 py-4   text-lg md:text-2xl uppercase" } },
    { label: dict.bonusPolicy.other.diligence.table.headers[1], props: { colSpan: 2, className: "border border-black text-center font-bold text-white   text-lg md:text-2xl py-4 uppercase w-fit lg:w-[500px]" } },
    { label: dict.bonusPolicy.other.diligence.table.headers[3], props: { rowSpan: 2, className: "border border-black text-center font-bold w-fit lg:w-[290px] px-6 py-4 text-white   text-lg md:text-2xl uppercase" } },
    {
      label: dict.bonusPolicy.other.diligence.table.headers[4],
      props: { rowSpan: 2, className: "border border-black text-center font-bold w-fit lg:w-60 p-4 text-white   text-lg md:text-2xl uppercase" },
    },
  ];
  const tableSubHeaders = [
    { label: dict.bonusPolicy.other.diligence.table.subHeaders[0], className: "border border-black text-center font-bold text-white w-fit lg:w-62.5 h-16   text-lg md:text-2xl uppercase p-6" },
    { label: dict.bonusPolicy.other.diligence.table.subHeaders[1], className: "border border-black text-center font-bold text-white w-fit lg:w-62.5 h-16   text-lg md:text-2xl uppercase p-6" },
  ];
